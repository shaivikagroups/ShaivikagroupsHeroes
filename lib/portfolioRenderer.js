...
function getTechIcon(tech) {
    ...
    // Map common variations to SkillIcons identifiers
    const skillMap = {
        'node.js': 'nodejs', 'next.js': 'nextjs', 'c++': 'cpp', 'c#': 'cs',
        'react native': 'react', 'tailwind css': 'tailwind', 'vue.js': 'vue',
        'machine learning': 'tensorflow', 'artificial intelligence': 'tensorflow',
        'deep learning': 'tensorflow', 'data science': 'python', 'sql': 'mysql',
        'html5': 'html', 'css3': 'css', 'jupyter': 'python'
    };
    
    // Remove non-alphanumeric chars for the API (e.g. "java 8" -> "java8")
    let iconName = skillMap[key] || key.replace(/[^a-z0-9]/g, '');
    
    return `<img src="https://skillicons.dev/icons?i=${encodeURIComponent(iconName)}" 
                 alt="${esc(tech)}" 
                 style="width:28px; height:28px; object-fit:contain; border-radius:4px;"
                 onerror="this.style.display='none'; this.nextElementSibling.style.display='inline-block';"
            /><span style="display:none; font-weight:800; font-size:1.2rem; color:var(--gold-dark);">${esc(tech.charAt(0).toUpperCase())}</span>`;
}
