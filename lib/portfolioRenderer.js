...
function generateResumeUrls(employee) {
    ...
    if (rawUrl.includes('cloudinary.com')) {
        ...
        // To view them inline without re-uploading, we route them through our secure backend proxy which overrides the headers.
        ...
        if (isRawUpload || (!isNewImagePdf && !rawUrl.toLowerCase().endsWith('.pdf'))) {
            viewUrl = `/api/proxy-resume?url=${encodeURIComponent(viewUrl)}`;
        }
    }
