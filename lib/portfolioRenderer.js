...
const TECH_ICONS = {
    ...
    react: '⚛️', 'react native': '📱', 'node.js': '🟢', nodejs: '🟢', node: '🟢', express: '🚂',
    html: '🌐', html5: '🌐', css: '🎨', css3: '🎨', tailwind: '💨', bootstrap: '🅱️',
    mongodb: '🍃', mongo: '🍃', postgresql: '🐘', postgres: '🐘', sql: '🗄️', mysql: '🐬', 
    firebase: '🔥', supabase: '⚡', docker: '🐳', kubernetes: '☸️', k8s: '☸️',
    aws: '☁️', gcp: '☁️', azure: '☁️', git: '🌿', github: '🐙', gitlab: '🦊',
    flutter: '💙', dart: '🎯', 'c++': '⚙️', cpp: '⚙️', c: '⚙️', 'c#': '🎵', csharp: '🎵',
    rust: '🦀', go: '🐹', golang: '🐹', php: '🐘', ruby: '💎', rails: '🛤️',
    swift: '🦅', kotlin: '🟣', r: '📊', matlab: '📐',
    tensorflow: '🧠', tf: '🧠', pytorch: '🔥', keras: '🔴', scikit: '🔬',
    'machine learning': '🤖', ml: '🤖', ai: '🧠', 'artificial intelligence': '🧠', 
    'deep learning': '🧠', nlp: '💬', 'data science': '📊', pandas: '🐼', numpy: '🔢',
    django: '🎸', flask: '🧪', fastapi: '⚡', spring: '🌱', 'spring boot': '🌱',
    vue: '💚', 'vue.js': '💚', angular: '🔴', svelte: '🧡', next: '▲', 'next.js': '▲', 
    figma: '🎨', xd: '🎨', sketch: '✏️', photoshop: '🖼️', illustrator: '✒️',
    blender: '🎨', unity: '🎮', 'unreal engine': '🎮',
    blockchain: '⛓️', solidity: '💎', ethereum: '💎', web3: '🌐',
    linux: '🐧', ubuntu: '🟠', bash: '💻', shell: '💻', powershell: '💙', 
    redis: '🔴', elasticsearch: '🔍', graphql: '🟣', rest: '🔗', api: '🔌',
    jira: '📋', agile: '🏃', scrum: '🏉', excel: '📗', word: '📘', powerpoint: '📙'
};
...
function getTechIcon(tech) {
    ...
    for (const [k, v] of Object.entries(TECH_ICONS)) {
        if (key.includes(k) && k.length > 2) return v;
    }
    
    // Fallback based on first letter
    const firstLetter = key.charAt(0);
    if (/[a-d]/i.test(firstLetter)) return '🔥';
    if (/[e-h]/i.test(firstLetter)) return '⚡';
    if (/[i-m]/i.test(firstLetter)) return '🚀';
    if (/[n-s]/i.test(firstLetter)) return '💡';
    return '🌟';
}
...
// ─── Resume URLs Builder ─────────────────────────────────────────────────────
function generateResumeUrls(employee) {
    let rawUrl = safeUrl(employee.resumeUrl);
    if (!rawUrl) return null;

    // 1. Strip Google redirect poisoning if present
    try {
        if (rawUrl.startsWith('https://www.google.com/url') || rawUrl.startsWith('http://www.google.com/url')) {
            const urlObj = new URL(rawUrl);
            const q = urlObj.searchParams.get('q');
            if (q) rawUrl = safeUrl(q) || rawUrl;
        }
    } catch (e) {
        // Ignore parsing errors and proceed with original
    }

    // Generate clean filename: "First-Last-Resume.pdf"
    const nameStr = (employee.fullName || 'Employee').trim().replace(/[^a-zA-Z0-9\s]/g, '');
    const cleanName = nameStr.split(/\s+/).join('-');
    ...
    if (rawUrl.includes('cloudinary.com')) {
        // Remove any existing fl_attachment if present in the raw URL
        viewUrl = rawUrl.replace(/fl_attachment:[^/]+\//, '').replace(/fl_attachment\//, '');
        
        // Inject fl_attachment for the explicit download URL
        if (rawUrl.includes('/upload/')) {
            downloadUrl = rawUrl.replace('/upload/', `/upload/fl_attachment:${filename}/`);
        } else {
            downloadUrl = rawUrl;
        }

        // Root Cause Fix for VIEWING inline:
        // Cloudinary forces Content-Disposition: attachment for ALL '/raw/' uploads (our legacy resumes).
        // The ONLY way to view them inline without re-uploading is to route through Google Docs Viewer.
        // New PDFs are uploaded as '/image/' which natively display in the browser.
        const isRawUpload = rawUrl.includes('/raw/upload/');
        const isNewImagePdf = rawUrl.includes('/image/upload/') && rawUrl.toLowerCase().endsWith('.pdf');
        
        if (isRawUpload || (!isNewImagePdf && !rawUrl.toLowerCase().endsWith('.pdf'))) {
            viewUrl = `https://docs.google.com/viewer?url=${encodeURIComponent(viewUrl)}&embedded=true`;
        }
    }
...
// ─── Nav Links ───────────────────────────────────────────────────────────────
function buildNavLinks(employee, allTech) {
    ...
    const resumeData = generateResumeUrls(employee);
    const resumeBtn = resumeData
        ? `<div class="pg-nav-resume-group">
               <a href="${esc(resumeData.viewUrl)}" target="_blank" rel="noopener noreferrer" class="pg-nav-resume pg-nav-resume-view" aria-label="View ${esc(employee.fullName)}'s resume inline">
                   View Resume <span style="font-size:0.85em; margin-left:4px;">↗</span>
               </a>
               <a href="${esc(resumeData.downloadUrl)}" target="_blank" rel="noopener noreferrer" class="pg-nav-resume pg-nav-resume-dl" aria-label="Download ${esc(employee.fullName)}'s resume">
                   <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" aria-hidden="true" style="margin-right:4px;"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                   Download <span style="font-size:0.85em; margin-left:2px;">↓</span>
               </a>
           </div>` : '';
    ...
    return { navHtml, resumeBtn, mobileLinks, resumeData };
}
...
function buildSocialButtons(employee) {
    ...
    const resumeData = generateResumeUrls(employee);
    if (resumeData) {
        buttons.push(`<a href="${esc(resumeData.viewUrl)}" target="_blank" rel="noopener noreferrer" class="pg-btn pg-btn-primary" aria-label="View ${esc(employee.fullName)}'s resume">
        ...
        buttons.push(`<a href="${esc(resumeData.downloadUrl)}" target="_blank" rel="noopener noreferrer" class="pg-btn pg-btn-outline" aria-label="Download ${esc(employee.fullName)}'s resume">
            <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
            Download Resume ↓
        </a>`);
    }
...
function generatePortfolioHTML(employee, portfolioUrl) {
    ...
    const { navHtml, resumeBtn, mobileLinks, resumeData } = buildNavLinks(employee, allTech);
...
<body>
    ...
    <div class="pg-mobile-menu" id="pg-mobile-menu" role="menu" aria-hidden="true">
        ${mobileLinks}
        ${resumeData ? `
            <div class="pg-mobile-resume-group" style="display:flex; flex-direction:column; gap:8px; margin-top:10px;">
                <a href="${esc(resumeData.viewUrl)}" target="_blank" rel="noopener noreferrer" class="pg-mobile-link" style="text-align:center; background:var(--primary-transparent); border-radius:8px;">View Resume ↗</a>
                <a href="${esc(resumeData.downloadUrl)}" target="_blank" rel="noopener noreferrer" class="pg-mobile-link" style="text-align:center; background:rgba(255,255,255,0.05); border-radius:8px;">Download Resume ↓</a>
            </div>
        ` : ''}
    </div>
